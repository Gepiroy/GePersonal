	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		if(sender instanceof Player){
			...
			if(!p.hasPermission("gepers.helper")){
				TextUtil.mes(p, pref, "Вы не можете банить. &aХотите стать &bхелпером&a? &fПроверьте команду &e/become");
				return true;
			}
		}
		if(args.length<4){
			TextUtil.mes(sender, pref, "&a/ban &"+GepUtil.boolString("a", "6", args.length>0)+"<Ник> &"+GepUtil.boolString("a", "6", args.length>1)+"<Срок> &"+GepUtil.boolString("a", "6", args.length>2)+"[Причины]");
			return true;
		}
		...
		tpi.addBan(ban.by, t, reason);//save conf и так здесь
		if(target!=null){
			ban.kick(target);
		}
		TextUtil.globMessage("&cБАН", "Игрока &e"+args[0]+"забанил &6"+sender.getName()+"&f на &6"+ban.remain()+"&f по причине &e"+reason+"&f.", Sound.BLOCK_ANVIL_PLACE, 2, 0, null, null, 0, 0, 0);
